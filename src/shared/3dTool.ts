import { ViewportPreset } from "@cornerstonejs/core/dist/types/types";
...
function load3DTool(
  ...
  if (onClick) {
    const button3D = document.getElementById("button-3d") as HTMLButtonElement;
    button3D.onclick = onClick;
  }
  const button3DSelect = document.getElementById(
    "button-3d-select"
  ) as HTMLUListElement;
  presets.forEach((element) => {
    ...
    option.onclick = () => {
      set3DPreset(option.textContent);
      button3DSelect.classList.toggle("show");
      if (onValueSelect) onValueSelect(option.textContent);
    };
    button3DSelect.appendChild(option);
  });
}

function set3DPreset(preset: string) {
  const button3DSelect = document.getElementById("button-3d-select");
  button3DSelect.dataset.preset = preset;
}
